...
\begin{abstract}
Sustainable agriculture (Theme T4) is critical for food security in Pakistan. This project, \textit{Agri-Intel}, addresses the lack of data-driven decision-making in fertilizer application. We integrated four heterogeneous data sources—government agricultural statistics, satellite imagery (MODIS NDVI), climate reanalysis data (NASA POWER), and geospatial vector data—to build a machine learning pipeline. Using a Gradient Boosting Regressor, we achieved high accuracy in predicting Wheat yield at the district level. Furthermore, we developed an end-to-end Proof of Concept (PoC) using Streamlit, featuring a geospatial dashboard and a ``What-If'' simulation engine that recommends optimal fertilizer levels to maximize yield. This system empowers stakeholders to move from reactive farming to predictive, precision agriculture.
\end{abstract}
...
Agriculture is the backbone of Pakistan's economy, yet it suffers from inefficiency. Farmers often apply fertilizer based on intuition rather than precise environmental requirements, leading to either yield gaps or environmental degradation through runoff. [cite_start]Theme T4 (Sustainable Agriculture \& Food Security) highlights the need for intelligent systems to mitigate these risks[cite: 82, 85].
...
We curated a dataset spanning 14 years (2002--2015) covering all districts of Punjab. [cite_start]The project satisfies the requirement of using 3-4 heterogeneous sources[cite: 110]:

\subsection{Data Sources}
\begin{enumerate}
    \item \textbf{Agricultural Statistics (Tabular):} Sourced from the Punjab Crop Reporting Service. Contains annual records of Wheat production, area sown, and fertilizer usage.
    \item \textbf{Remote Sensing (Raster):} MODIS Terra (MOD13A2) 16-day composite data. [cite_start]We extracted the Mean NDVI (Normalized Difference Vegetation Index) for the growing season (Oct--Mar) to proxy crop vigor[cite: 90].
    \item \textbf{Climate Data (Time-Series API):} Sourced from NASA POWER. [cite_start]We fetched daily precipitation and temperature data, aggregated into seasonal totals (Total Rainfall) and averages (Avg Temp)[cite: 57].
    \item \textbf{Geospatial Boundaries (Vector):} GeoJSON files of Pakistan's administrative districts were used to spatially aggregate the satellite and climate data.
\end{enumerate}

\subsection{Integration Strategy}
A major challenge was the lack of a common key. We developed a robust cleaning pipeline to standardize district names (e.g., removing suffixes like ``District'' or ``Divn''). We calculated the geometric centroids of district polygons to query the NASA API. Finally, all sources were merged into a single master dataset keyed by \texttt{District\_ID} and \texttt{Year}.
...
The Gradient Boosting model outperformed the baseline significantly, capturing the complex interaction between weather stress and fertilizer response.
...
As shown in Fig. \ref{fig:feat_imp}, Fertilizer Usage and Area Sown were dominant predictors, but Environmental factors (NDVI and Temperature) played a crucial role in refining the predictions, validating our multi-source approach.
...
[cite_start]We developed a fully functional web application using \textbf{Streamlit} and \textbf{Folium}[cite: 128].

\subsection{Interactive Geospatial Dashboard}
The dashboard (Fig. \ref{fig:error}) features an interactive map of Punjab. Users can select a district, and the system automatically fetches historical defaults and real-time weather context (via OpenWeatherMap API) for that location.
...
\caption{PoC: Error Analysis of the Yield Prediction Model.}
\label{fig:error}
\end{figure}
